...
def download_from_twitter(url: str, output_dir: str = ".") -> Optional[Dict]:
    ...
    # yt-dlp command with browser cookies for content-warned videos
    cmd = [
        "yt-dlp",
        "-f", "best[ext=mp4]",
        "--cookies-from-browser", "chrome",  # Uses Chrome cookies (change to "firefox" or "safari" if needed)
        "-o", str(output_path),
        url
    ]
...
- Understand CONTEXT: when a journalist shows tattered tents and cold families, they're documenting displacement
- READ BETWEEN THE LINES: what is the journalist witnessing and why are they reporting it?
- Connect journalist observations to actual events/conditions they're documenting
...
**CATEGORIES** (choose exactly ONE that best matches what's being DOCUMENTED):
{json.dumps(CATEGORIES, indent=2)}

**TAGS** (choose ALL that apply to what's being SHOWN/DOCUMENTED):
{json.dumps(TAGS, indent=2)}

**REASONING REQUIREMENTS:**
- Focus on what EVENT/SITUATION is being documented, not what words were said
- Make clear INFERENCES about what the journalist is witnessing
- Explain WHY this reporting indicates the chosen category
- Use confidence levels based on clarity of evidence, not explicitness of language
